package com.xiyuan.taobao.api.internal.parser.json;
...
import com.xiyuan.taobao.api.Constants;
import com.xiyuan.taobao.api.internal.mapping.Converter;
import com.xiyuan.taobao.api.internal.mapping.Converters;
import com.xiyuan.taobao.api.internal.mapping.Reader;
import com.xiyuan.taobao.api.internal.util.json.ExceptionErrorListener;
import com.xiyuan.taobao.api.internal.util.json.JSONReader;
...
public class SimplifyJsonConverter implements Converter {
	...
	public <T> T toResponse(String rsp, Class<T> clazz) throws ApiException {
		...
		if (rootObj instanceof Map<?, ?>) {
			...
			if(Constants.RESPONSE_TYPE_QIMEN.equals(responseType)) {
				Object errorJson = rootJson.get(Constants.QIMEN_CLOUD_ERROR_RESPONSE);
				if (errorJson instanceof Map<?, ?>) {
					return fromJson((Map<?, ?>) errorJson, clazz);
				} else {
					return fromJson(rootJson, clazz);
				}
			} else if(Constants.RESPONSE_TYPE_QIMEN2.equals(responseType)
					|| Constants.RESPONSE_TYPE_DINGTALK_OAPI.equals(responseType)) {
					return fromJson(rootJson, clazz);
			} else {
